...
class Discriminator(nn.Module):
  ...
  def __init__(self, D_ch=64, D_wide=True, resolution=128,
               ...
               D_lr=2e-4, D_B1=0.0, D_B2=0.999,E_lr=5e-5, G_B1=0.0, G_B2=0.999, adam_eps=1e-8,
               SN_eps=1e-12, output_dim=1, D_mixed_precision=False, D_fp16=False,
               D_init='ortho', skip_init=False, D_param='SN',AC=False,Arc=False,dim_z=128,E=False, **kwargs):
    ...
    self.blocks = []
    for index in range(len(self.arch['out_channels'])):
      self.blocks += [[layers.DBlock(E=self.E, in_channels=self.arch['in_channels'][index],
                       out_channels=self.arch['out_channels'][index],
                       which_conv=self.which_conv,
                       wide=self.D_wide,
                       activation=self.activation,
    ...
    if self.AC:
        if self.Arc:
            self.linear_mi = layers.ArcMarginProduct(self.arch['out_channels'][-1], n_classes)
            self.linear_c = layers.ArcMarginProduct(self.arch['out_channels'][-1], n_classes)
        else:
    ...
    self.BN_C = nn.BatchNorm1d(self.arch['out_channels'][-1])
    # Initialize weights
    if not skip_init:
      self.init_weights()
    ...
    # Set up optimizer
    if self.E:
        self.lr, self.B1, self.B2, self.adam_eps = E_lr, G_B1, G_B2, adam_eps
    else:
  ...
  def forward(self, x, z=None, y=None):
    ...
    if self.E:
        return self.linear_e(h)
    else:
        out_x = self.linear_x(h)
        h = torch.cat([h, self.linear_z(z)], dim=1)
        # Get initial class-unconditional output
        out_xz = self.linear_xz(h)
        out = out_x + out_xz
        ...
        if self.AC:
            h = self.BN_C(h)
            if self.Arc:
                out_mi = self.linear_mi(h, y)
                out_c = self.linear_c(h, y)
            else:
