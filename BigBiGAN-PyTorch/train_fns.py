...
def GAN_training_function(G, D,BI, GD, z_, y_, ema, state_dict, config):
  def train(x, y):
    G.optim.zero_grad()
    D.optim.zero_grad()
    torchvision.utils.save_image(x.float().cpu(), 'real_image.png',
    ...
    for step_index in range(config['num_D_steps']):
      ...
      D.optim.zero_grad()
      for accumulation_index in range(int(config['num_D_accumulations']/2)):
        z_.sample_()
        y_.sample_()
        # print(y_)
    ...
    for step_index in range(config['num_G_steps']):
        ...
        for accumulation_index in range(int(config['num_G_accumulations']/2)):
            z_.sample_()
            y_.sample_()
