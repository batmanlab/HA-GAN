...
import inception_model_vggface
import torchvision
...
def prepare_inception_metrics(dataset, parallel, no_fid=False):
  ...
  if 'VGGFACE' in dataset:
    vgg_num_classes = int(dataset.strip('VGGFACE'))
    inception_dict = torchvision.models.inception_v3(pretrained=False)
    inception_dict.fc = nn.Linear(2048, vgg_num_classes)
    net = inception_model_vggface.WrapInception(net=inception_dict).cuda().eval()
    net.load_state_dict(torch.load('VGGFACE' + str(vgg_num_classes) + '.pth'))
  else:
    net = load_inception_net(parallel)

  print('load inception model', (dataset  + '.pth'))
