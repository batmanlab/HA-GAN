...
def main():
    ...
    latent_dim = 1024
    basename = str(img_size)+"_"+str(latent_dim)+"_COPD"
    SAVE_MODEL = True
    ...
    for iteration in range(continue_step, TOTAL_ITER):
        ...
        if iteration%200 == 0:
            ...
            fig=plt.figure()
            plotting.plot_img(featmask,title="REAL",cut_coords=(128,128,16),figure=fig,draw_cross=False,cmap="gray")
            summary_writer.add_figure('Real', fig, iteration, close=True)

            featmask = np.squeeze((0.5*sub_x_hat_rec[0]+0.5).data.cpu().numpy())
            featmask = nib.Nifti1Image(featmask.transpose((2,1,0)),affine = np.eye(4))
            fig=plt.figure()
            plotting.plot_img(featmask,title="REC",cut_coords=(128,128,16),figure=fig,draw_cross=False,cmap="gray")
            summary_writer.add_figure('Rec', fig, iteration, close=True)
            
            featmask = np.squeeze((0.5*fake_images[0]+0.5).data.cpu().numpy())
            featmask = nib.Nifti1Image(featmask.transpose((2,1,0)),affine = np.eye(4))
            fig=plt.figure()
            plotting.plot_img(featmask,title="FAKE",cut_coords=(128,128,16),figure=fig,draw_cross=False,cmap="gray")
            summary_writer.add_figure('Fake', fig, iteration, close=True)
            
        if SAVE_MODEL and iteration > 30000 and (iteration+1)%500 == 0:
