...
class Generator(nn.Module):
    ...
    def forward(self, h, crop_idx, return_latent=False, return_feature=False, inter_feature=False, latent_only=False):

        if crop_idx != None:
            if inter_feature==False:
                h = self.fc1(h)
                if return_feature:
                    return h
            ...
            h = F.interpolate(h,scale_factor = 2) # Out: (8, 8, 8), 512
            h = self.tp_conv2(h)
            h = self.relu(self.bn2(h))

            h = F.interpolate(h,scale_factor = 2) # Out: (16, 16, 16), 512
            h = self.tp_conv3(h)
            h = self.relu(self.bn3(h))

            h = F.interpolate(h,scale_factor = 2) # Out: (32, 32, 32), 256
            h = self.tp_conv4(h)
            h = self.relu(self.bn4(h))

            h = F.interpolate(h,scale_factor = 2) # Out: (64, 64, 64), 128
            h = self.tp_conv5(h)
            h_latent = self.relu(self.bn5(h)) # (64, 64, 64), 64
            if latent_only:
                return h_latent
            if self.mode == "train":
        ...
        h1 = self.tp_conv6(h1)
        h1 = self.relu(self.bn6(h1)) # (128, 128, 128), 32
