...
class Lung(torch.utils.data.Dataset):
  def __init__(self, root, transform=None, target_transform=None,
    ...
    data = np.loadtxt("/pghbio/dbmi/batmanlab/juc91/rep/lung_image_BoW/img_size.txt", str)
    ids = data[:, 0]
    sizes = np.array(data[:, 3], int)

    n_processed = 0
    self.id_dict = dict()
    for iii in range(len(ids)):
      for jjj in range(sizes[iii]):
        self.id_dict[n_processed + jjj] = (ids[iii], jjj)
      n_processed += sizes[iii]

    N = 3023040

    if self.train:
      self.N = 2417470
      self.data_dir = "/pghbio/dbmi/batmanlab/juc91/rep/lung_image_BoW/train/"
    else:
      self.N = N - 2417470
      self.data_dir = "/pghbio/dbmi/batmanlab/juc91/rep/lung_image_BoW/test/"

    self.transform = torchvision.transforms.Resize((256, 256))

    self.upsampling = nn.modules.upsampling.Upsample(scale_factor=8, mode="bilinear")
  ...
  def __getitem__(self, index):
    ...
    res1 = self.transform(transforms.ToPILImage()(torch.as_tensor(np.float32(img))))
    res[0, :, :] = (transforms.ToTensor()(res1) - 0.5) * 2
    res[1, :, :] = (transforms.ToTensor()(res1) - 0.5) * 2
    res[2, :, :] = (transforms.ToTensor()(res1) - 0.5) * 2
