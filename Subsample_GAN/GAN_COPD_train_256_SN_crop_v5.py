...
from Model_GAN_256_SN_v5 import Discriminator, Generator
...
def main():
    ...
    latent_dim = 1000
    basename = str(img_size)+"_"+str(latent_dim)+"_MSE_SN_v5"
    SAVE_MODEL = True
    ...
    for iteration in range(TOTAL_ITER):
        ...
        y_real_pred = D(real_images_crop, real_images_small)
        ...
        y_fake_pred = D(fake_images.detach(), fake_images_small.detach())
        ...
        for iters in range(g_iters):
            ...
            noise = torch.randn((BATCH_SIZE, latent_dim, 1, 1, 1)).cuda()
            fake_images, fake_images_small = G(noise, crop_idx)
            y_fake_g = D(fake_images, fake_images_small)
